...
@post('/api/blogs')
# 新建博客
async def api_create_blog(request, *, name, sub_name, summary, content):
    check_admin(request)
    if not name or not name.strip():
    ...
    blog = Blog(user_id=request.__user__.id, user_name=request.__user__.name, user_image=request.__user__.image, name=name.strip(), sub_name=sub_name.strip(), summary=summary.strip(), content=content.strip())
    await blog.save()
    return blog
